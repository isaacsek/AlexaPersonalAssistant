...
var BookmarkedObj = {
    "home": "47.830002,-122.234794",
    "school": "47.759199,-122.191848",
    "work": "47.777843,-122.183394",
    "chelsea's": "47.497250,-122.313322"
};

// User and Google API configuration related variables
// 1. Setting Coordinates for your home / origin
var user_origin = "47.655932,-122.309258"; // can link account
...
var google_api_key = "AIzaSyDC1vpiUx3IOcLPv0DDMQqZzKHoWhZJW_Q"; // CHANGE IT WITH YOUR API KEY
...
function getData(options, callback) {
    ...
    https.get(options, function(res) {
        // keeps on calling data
        res.on("data", function(chunk) {
            text += chunk.toString('utf-8');
        });

        // when completed, end will be called
        res.on('end', function() {
            return callback(text); // complete JSON text from google
        });
    }).on('error', function(e) {
...
var handlers = {
    ...
    'Introduction': function() {
        ...
        // reprompt is when intro is given, but nothing is said afterwards, hence reprompt
      ...
      getData(options, function (jsondata) {
          ...
          if (jsondata == 'ERROR') {
              ...
              if (status == "OK") {
                  ...
                  var timeinhhmm = ld.toLocaleTimeString("en-US", {timeZone: 'America/Los_Angeles', hour: '2-digit', minute:'2-digit'});
                  //var timeinhhmm = ld.toLocaleTimeString("en-US", {timeZone: 'Asia/Kolkata', hour:'2-digit', minute:'2-digit'});
                  // https://en.wikipedia.org/wiki/List_of_tz_database_time_zones // America/Los_Angeles

                  var speechOutput = "It will take you " + duration + " to arrive at " + speakdestination + ". You will arrive around <say-as interpret-as=\"time\">" + timeinhhmm + "</say-as> if you leave within 5 minutes.";
                  self.emit(":tell", speechOutput);
